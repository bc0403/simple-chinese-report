...
% \hypersetup{hidelinks}  % using `colorlink` for electronic reading or `hidelinks` for print
\crefname{figure}{图}{图}
\crefname{table}{表}{表}
...
% 设置参考文献样式
\bibliographystyle{IEEEtran}  % unsrt for generic purpose  
...
实验设计部分应该包括实验对象、实验设备、实验步骤等内容。实验的整体流程如\cref{fig:workflow} 所示，该流程图清晰地展示了从数据准备到结果分析的完整过程。
...
实验的具体参数设置如\cref{tab:parameters} 所示，这些参数的选择基于前期预实验结果。
...
可以在这里展示实验数据、图表等。如\cref{fig:example} 所示，我们使用了示例图片来展示实验结果的可视化表示。该图片清晰地展示了实验数据的分布情况。
...
对实验结果进行统计分析，验证假设或得出结论。\cref{tab:example} 展示了实验组和对照组的详细数据对比。从表中可以看出，实验组的各项指标均优于对照组。
...
通过对比\cref{fig:example} 和\cref{tab:example} 中的数据，我们可以得出初步的实验结论。

\subsection{性能对比}

为了进一步验证方法的有效性，我们进行了多组对比实验。\cref{fig:performance} 展示了不同方法在测试集上的性能对比结果。
...
从 \cref{fig:performance} 可以看出，本文提出的方法在各项指标上均优于传统方法。具体数值对比见\cref{tab:comparison}。
...
简要总结研究的主要内容和重要发现。基于\cref{fig:workflow} 所示的实验流程和\cref{tab:parameters} 中的参数设置，我们完成了系统的实验验证。实验结果表明，如\cref{fig:example} 和\cref{fig:performance} 所示，本文提出的方法在多个方面都取得了显著改进。\cref{tab:comparison} 的详细数据对比进一步证实了方法的有效性。

\subsection{研究展望}

提出未来可以进一步研究的方向和问题。虽然本文的实验结果令人满意，但仍存在一些局限性。例如，在\cref{tab:example} 中展示的数据规模相对较小，未来可以考虑扩大数据集规模。同时，基于\cref{fig:performance} 中观察到的性能趋势，可以探索更复杂的模型结构来进一步提升性能。
